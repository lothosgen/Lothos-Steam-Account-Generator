...
namespace SteamAccCreator.Web
{
    public class HttpHandler
    {
        ...
        public bool CreateAccount(string email, string captchaText, ref string status, string proxy, bool _useProxy)
        {
            _proxy = proxy;
        ...
        public bool CheckEmailVerified(ref string status, string proxy, bool _useProxy)
        {
        ...
        public bool CompleteSignup(string alias, string password, ref string status, string proxy, bool _useProxy)
        {
            if (!CheckAlias(alias, ref status, proxy, _useProxy))
                return false;
            if (!CheckPassword(password, alias, ref status, proxy, _useProxy))
                return false;
        ...
        private static bool CheckAlias(string alias, ref string status, string proxy, bool _useProxy)
        {
            ...
            if (_useProxy)
            {
                tempClient.Proxy = new WebProxy(Regex.Split(proxy, ":")[0], int.Parse(Regex.Split(proxy, ":")[1]));
            }
        ...
        private static bool CheckPassword(string password, string alias, ref string status, string proxy, bool _useProxy)
        {
